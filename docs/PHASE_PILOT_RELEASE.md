...
- **CI: confirmed.** A GitHub Actions workflow running `make check` and `npm run check` on every push/PR, then deploying to staging automatically on merge to `main` (per §2's confirmed release shape), with production as a separate manual/tagged promotion. Costs nothing at this repo's size and catches regressions before they reach either VPS. Dependency scanning via `govulncheck`/`npm audit` (or GitHub's own Dependabot) substitutes for §17's "container scanning," since Phase 9 deliberately has no container to scan.
