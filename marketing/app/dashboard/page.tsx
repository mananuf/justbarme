...
import { AppBottomNav } from "@/components/app-shell"
import { Logo } from "@/components/logo"

function SyncDot() {
  return (
    <span className="relative flex h-2 w-2">
      <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-jb-gold opacity-50" />
      <span className="relative inline-flex rounded-full h-2 w-2 bg-jb-gold" />
    </span>
  )
}
    ...
    <div className="min-h-screen bg-jb-cream text-jb-ink pb-28">
      <div className="max-w-md mx-auto px-5 pt-6">
        <div className="flex items-center justify-between mb-5">
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-jb-ink/50 bg-white/70 rounded-full px-3 py-1.5 border border-jb-ink/10">
            <SyncDot />
            Waiting to sync
          </div>
        </div>
        ...
        {showGuide && (
          <div className="mb-5 rounded-xl border border-jb-ink/10 bg-white/70 px-4 py-3 flex items-start justify-between gap-3">
            ...
            <button onClick={() => setShowGuide(false)} className="text-jb-ink/30 hover:text-jb-ink text-sm shrink-0">✕</button>
          </div>
        )}
        ...
        <div className="rounded-2xl bg-jb-green-dark text-jb-cream p-5 mb-4">
          ...
          <div className="text-[34px] font-light mt-1" style={{ fontFamily: "var(--font-display)" }}>₦48,200</div>
        ...
        <div id="stock" className="grid grid-cols-2 gap-3 mb-4 scroll-mt-20">
          <div className="rounded-xl bg-white/70 border border-jb-ink/[0.08] p-4">
            ...
            <div className="mt-2 inline-block text-[10px] px-2 py-0.5 rounded-full bg-jb-gold/20 text-[#7a5b1f] font-medium">Gordon&apos;s 75cl</div>
          </div>
          <div id="bills" className="rounded-xl bg-white/70 border border-jb-ink/[0.08] p-4 scroll-mt-20">
            ...
            <div className="mt-2 inline-block text-[10px] px-2 py-0.5 rounded-full bg-jb-ink/[0.06] text-jb-ink/60 font-medium">2 open tabs</div>
          </div>
        </div>
        ...
        <div id="more" className="scroll-mt-20">
          ...
          {moreOpen && (
            <div className="mt-2 rounded-xl border border-jb-ink/10 bg-white/60 divide-y divide-jb-ink/[0.06] overflow-hidden">
              ...
              <Link href="/install" className="px-4 py-3.5 flex flex-col hover:bg-jb-ink/[0.03] transition-colors">
                <span className="text-[13px] text-jb-ink/75">Settings — Install justbarme</span>
                <span className="text-[11px] text-jb-ink/40">Add justbarme to your home screen</span>
              </Link>
            </div>
          )}
        </div>
