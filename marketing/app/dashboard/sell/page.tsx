  ...
  if (done) {
    return (
      <div className="min-h-screen bg-jb-cream text-jb-ink flex flex-col items-center justify-center px-6 text-center pb-28">
        <div className="w-16 h-16 rounded-full bg-jb-ink text-jb-cream flex items-center justify-center text-2xl mb-6" style={{ animation: "pop-in 0.5s cubic-bezier(0.16,1,0.3,1) both" }}>
          ✓
        </div>
        <h1 className="text-2xl font-light tracking-tight mb-2" style={{ fontFamily: "var(--font-display)" }}>Sale recorded</h1>
        ...
        <Link href="/dashboard" className="rounded-xl bg-jb-ink text-jb-cream text-[15px] font-medium py-4 px-8 hover:bg-jb-green transition-colors">
          Back to Dashboard
        </Link>
        <AppBottomNav />
      </div>
            ...
            return (
              <button
                ...
                className={`rounded-xl border p-4 text-left transition-colors active:scale-[0.97] ${
                  inCart ? "border-jb-ink bg-jb-ink text-jb-cream" : "border-jb-ink/10 bg-white text-jb-ink"
                }`}
              >
                <div className="text-[14px] font-medium">{p.name}</div>
                <div className={`text-[11px] mt-0.5 ${inCart ? "text-jb-cream/60" : "text-jb-ink/45"}`}>{p.size} · ₦{p.price}</div>
                {inCart && (
        ...
        {cart.length > 0 && (
          <div className="mt-6">
            ...
            <div className="space-y-2">
              {cart.map((line) => (
                <div key={line.name} className="flex items-center justify-between rounded-xl border border-jb-ink/10 bg-white px-4 py-3">
                  <div>
                    <div className="text-[13.5px] text-jb-ink/80">{line.name}</div>
                    <div className="text-[11px] text-jb-ink/40">₦{line.price} each</div>
                  </div>
                  <div className="flex items-center gap-3">
                    <button onClick={() => changeQty(line.name, -1)} className="w-7 h-7 rounded-full border border-jb-ink/20 flex items-center justify-center text-jb-ink">−</button>
