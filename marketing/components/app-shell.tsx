...
function Glyph({ icon, active, onDark }: { icon: string; active?: boolean; onDark?: boolean }) {
  ...
  const common = { width: 19, height: 19, viewBox: "0 0 24 24", fill: "none", stroke, strokeWidth: 1.7 } as const
  switch (icon) {
    case "home":
    ...
    default:
      return <svg {...common}><circle cx="5" cy="12" r="1.5" fill={stroke} /><circle cx="12" cy="12" r="1.5" fill={stroke} /><circle cx="19" cy="12" r="1.5" fill={stroke} /></svg>
  }
}
  ...
  return (
    <nav className="fixed bottom-0 inset-x-0 z-40 border-t border-jb-ink/[0.08] bg-jb-cream/95 backdrop-blur px-2 pt-2 pb-[calc(env(safe-area-inset-bottom)+0.5rem)]">
      <div className="max-w-md mx-auto flex items-stretch justify-between">
        {NAV_ITEMS.map((item) => {
          const active = item.label === "Home" ? pathname === "/dashboard" : item.label === "Sell" ? pathname.startsWith("/dashboard/sell") : false
          const isSell = item.label === "Sell"
          return (
            <Link key={item.label} href={item.href} className="flex-1 flex flex-col items-center gap-1 py-1">
              <div
                className={
                  isSell
              ...
              <span className={`text-[10px] ${active ? "text-jb-ink font-medium" : "text-jb-ink/40"}`}>{item.label}</span>
            </Link>
          )
        })}
      </div>
