...
type IconType = "notebook" | "sell" | "stock" | "bills" | "offline" | "reports" | "catalogue" | "people"
...
export function PixelIcon({ type, size = 40 }: PixelIconProps) {
  ...
  useEffect(() => {
    ...
    const draw = (t: number) => {
      ...
      switch (type) {
        case "notebook": drawNotebook(ctx, size, t); break
        case "sell": drawSell(ctx, size, t); break
        case "stock": drawStock(ctx, size, t); break
        case "bills": drawBills(ctx, size, t); break
        case "offline": drawOffline(ctx, size, t); break
        case "reports": drawReports(ctx, size, t); break
        case "catalogue": drawCatalogue(ctx, size, t); break
        case "people": drawPeople(ctx, size, t); break
      }
