...
function SyncDot({ state = "synced" }: { state?: "synced" | "pending" | "offline" }) {
  const color = state === "synced" ? "bg-emerald-600" : state === "pending" ? "bg-jb-gold" : "bg-jb-ink/30"
  return (
      ...
      {items.map((item) => {
        ...
        return (
          <div key={item.label} className="flex-1 flex flex-col items-center gap-1">
            ...
            <span className={`text-[9px] ${isActive ? "text-jb-ink font-medium" : "text-jb-ink/40"}`}>{item.label}</span>
          </div>
        )
      })}
    </div>
...
function NavGlyph({ icon, active, onDark }: { icon: string; active?: boolean; onDark?: boolean }) {
  ...
  const common = { width: 15, height: 15, viewBox: "0 0 24 24", fill: "none", stroke, strokeWidth: 1.8 } as const
  switch (icon) {
    case "home":
    ...
    case "stock":
      return <svg {...common}><rect x="3" y="9" width="8" height="11" rx="1" /><rect x="13" y="4" width="8" height="16" rx="1" /></svg>
    case "bills":
      return <svg {...common}><rect x="5" y="3" width="14" height="18" rx="2" /><path d="M9 8h6M9 12h6M9 16h3" /></svg>
    default:
      return <svg {...common}><circle cx="5" cy="12" r="1.4" fill={stroke} /><circle cx="12" cy="12" r="1.4" fill={stroke} /><circle cx="19" cy="12" r="1.4" fill={stroke} /></svg>
  }
}

function Pill({ children, tone = "neutral" }: { children: React.ReactNode; tone?: "neutral" | "warn" | "good" }) {
  const tones = {
    neutral: "bg-jb-ink/[0.06] text-jb-ink/60",
    warn: "bg-jb-gold/20 text-[#7a5b1f]",
    good: "bg-emerald-600/10 text-emerald-700",
  }
  return <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium tracking-wide ${tones[tone]}`}>{children}</span>
}
      ...
      <div className="px-4 mt-3 flex-1">
        ...
        <div className="space-y-1.5">
          {[
            ...
            <div key={row.what} className="flex items-center justify-between rounded-lg px-2.5 py-2 bg-white/50">
              <div>
    ...
    <div className="flex flex-col h-full">
        ...
        {DRINKS.map((d, i) => (
          <button
            ...
            className={`rounded-xl border p-2.5 text-left transition-colors ${
              i === selected ? "border-jb-ink bg-jb-ink text-jb-cream" : "border-jb-ink/[0.1] bg-white/70 text-jb-ink"
            }`}
          >
            <div className="text-[12px] font-medium">{d.name}</div>
            <div className={`text-[10px] ${i === selected ? "text-jb-cream/60" : "text-jb-ink/45"}`}>{d.size} · ₦{d.price}</div>
          </button>
        ))}
      </div>
      ...
      <div className="px-4 mt-4">
        <div className="rounded-xl border border-jb-ink/[0.1] bg-white/70 p-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setQty((q) => Math.max(1, q - 1))} className="w-7 h-7 rounded-full border border-jb-ink/20 flex items-center justify-center text-jb-ink">−</button>
            <span className="text-[14px] font-medium w-4 text-center">{qty}</span>
            <button onClick={() => setQty((q) => q + 1)} className="w-7 h-7 rounded-full border border-jb-ink/20 flex items-center justify-center text-jb-ink">+</button>
          </div>
        </div>
      </div>
      ...
      <div className="px-4 mt-auto pb-4 pt-4">
        <div className="flex items-center justify-between text-[12px] text-jb-ink/60 mb-2">
          <span>Total</span>
          <span className="text-jb-ink font-medium text-[14px]">₦{(DRINKS[selected].price * qty).toLocaleString()}</span>
        </div>
...
export function StockReceivePreview() {
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 mt-auto pb-4 pt-4">
        <button className="w-full rounded-xl bg-jb-ink text-jb-cream text-[13px] font-medium py-3">Save Receipt</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 mt-auto pb-4 pt-4 flex gap-2">
        <button className="flex-1 rounded-xl border border-jb-ink/15 text-jb-ink text-[12.5px] font-medium py-3">Share Bill</button>
        <button className="flex-1 rounded-xl bg-jb-ink text-jb-cream text-[12.5px] font-medium py-3">Record Payment</button>
      </div>
    </div>
  )
}
    ...
    <div className="flex flex-col h-full items-stretch px-4 pt-4 pb-4">
      <div className="rounded-2xl border border-jb-ink/[0.1] bg-white p-5 flex-1 flex flex-col">
        <div className="text-center">
          ...
          <div className="text-[10px] text-jb-ink/40 mt-1">14 Adeola Street, Lekki · 0803 000 0000</div>
        </div>
        <div className="mt-4 border-t border-dashed border-jb-ink/15 pt-3 space-y-1.5 text-[11.5px]">
          <div className="flex justify-between text-jb-ink/70"><span>Hennessy VS ×2</span><span>₦18,000</span></div>
          <div className="flex justify-between text-jb-ink/70"><span>Sprite ×3</span><span>₦1,500</span></div>
          <div className="flex justify-between text-jb-ink/50"><span>Paid</span><span>− ₦8,100</span></div>
        </div>
      ...
      <button className="mt-3 w-full rounded-xl bg-jb-ink text-jb-cream text-[13px] font-medium py-3">Share Bill</button>
    </div>
  )
}
...
export function CatalogueSetupPreview() {
  ...
  return (
    <div className="flex flex-col h-full">
          ...
          return (
            <button
              ...
              className={`px-3 py-2 rounded-full text-[12px] border transition-colors flex items-center gap-1.5 ${
                active ? "bg-jb-ink border-jb-ink text-jb-cream" : "border-jb-ink/15 text-jb-ink/60 bg-white/60"
              }`}
            >
      ...
      <div className="px-4 mt-auto pb-4 pt-4">
        <button className="w-full rounded-xl bg-jb-ink text-jb-cream text-[13px] font-medium py-3">Continue</button>
      </div>
    </div>
  )
}
...
export function ActivityPreview() {
  const rows = [
    ...
    { who: "System", what: "flagged a stock count mismatch", time: "7:20pm", tone: "warn" as const },
