...
const FEATURES: { icon: FeatureIconType; title: string; desc: string }[] = [
  ...
  },
  {
    icon: 'reports',
    title: 'Reports',
    desc: 'Understand daily sales, stock movement and expenses at a glance.',
  },
  {
    icon: 'offline',
    title: 'Offline Mode',
    desc: 'Keep recording operations even when the internet goes down.',
  },
];
...
const STEPS = [
  {
    ...
    desc: "Enter your business name and basic details. That's it to get started.",
  },
  {
    n: '02',
    title: 'Choose what you sell',
    desc: 'Select common drinks from the catalogue, or add your own products.',
  },
  {
    n: '03',
  ...
  },
  {
    ...
    desc: 'Record sales, stock, bills and expenses from your phone. Online or offline.',
  },
];
    ...
    <div className="bg-jb-cream text-jb-ink min-h-screen font-sans antialiased">
      ...
      <section className="relative px-6 md:px-12 lg:px-20 pt-28 pb-16 md:pt-40 md:pb-24 overflow-hidden">
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-14 items-center">
          <div>
            ...
            <p
              className="mt-6 text-base md:text-lg text-jb-ink/55 leading-relaxed max-w-md"
              style={reveal(120)}
            >
              Keep track of sales, stock, customer bills, expenses and staff activity from one
              simple app built for bars and lounges.
            </p>
            <div className="mt-8 flex flex-col sm:flex-row gap-3" style={reveal(200)}>
              <Link
                to="/onboarding"
                className="px-7 py-3.5 rounded-xl bg-jb-ink text-jb-cream text-sm font-medium text-center hover:bg-jb-green transition-colors active:scale-[0.98]"
              >
                Get Started
              </Link>
              <a
                href="#how-it-works"
                className="px-7 py-3.5 rounded-xl border border-jb-ink/15 text-jb-ink text-sm font-medium text-center hover:bg-jb-ink/[0.04] transition-colors"
              >
                See How It Works
              </a>
            </div>
            <div className="mt-10 flex flex-wrap gap-x-6 gap-y-2.5" style={reveal(280)}>
              {['Works offline', 'Set up in minutes', 'Built for Android'].map((label) => (
                <div
                  key={label}
                  className="flex items-center gap-2 text-xs text-jb-ink/45 tracking-wide"
                >
        ...
        <div className="max-w-6xl mx-auto">
          <div className="mb-14 max-w-2xl">
            ...
            <p className="mt-5 text-sm text-jb-ink/50 leading-relaxed">
              Set it up once. Run it in a few taps. justbarme quietly connects the everyday parts of
              running a bar so nothing falls through the cracks.
            </p>
          </div>
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              {FLOW.map((step, i) => (
                <React.Fragment key={step}>
                  <div className="px-4 py-2.5 rounded-xl bg-jb-ink text-jb-cream text-sm font-medium tracking-wide">
                    {step}
                  </div>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
            {FEATURES.map((f, i) => (
              <BentoCard
                key={f.title}
                className="p-6 min-h-[168px] flex flex-col justify-between"
                delay={i * 60}
              >
      ...
      <section className="py-24 md:py-32 px-6 md:px-12 lg:px-20 border-t border-jb-ink/[0.06]">
        <div className="max-w-6xl mx-auto">
          <SectionIntro
            icon="people"
            tag="MADE TO FIT"
            title={'Your bar works differently.\njustbarme adapts.'}
          />
          <div className="flex flex-wrap gap-2.5 mb-10">
            {CUSTOMIZATIONS.map((c) => (
              <span
                key={c}
                className="px-4 py-2 rounded-full border border-jb-ink/10 bg-white/60 text-[13px] text-jb-ink/65"
              >
                {c}
              </span>
            ))}
          ...
          <div>
            ...
            <ul className="mt-6 space-y-2.5">
              {[
                ...
                'Set your own prices',
                'Add custom products anytime',
                "Hide products you don't sell",
              ].map((x) => (
      ...
      <section className="py-24 md:py-32 px-6 md:px-12 lg:px-20 border-t border-jb-ink/[0.06]">
        <div className="max-w-6xl mx-auto">
          <SectionIntro
            icon="sell"
            tag="DESIGNED FOR THE BUSY NIGHT"
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
            {OPERATIONS.map((step, i) => (
              <BentoCard key={step.n} className="p-7 min-h-[180px] flex flex-col" delay={i * 80}>
                <span className="font-pixel text-[11px] text-jb-ink/25 tracking-widest block mb-6">
                  {step.n}
                </span>
          ...
          <p className="mt-6 text-sm text-jb-ink/40 max-w-lg">
            Checking today&apos;s sales is even simpler: open the dashboard, see the summary. No
            taps required.
          </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mt-16 max-w-2xl mx-auto">
            ...
            <div>
              <p className="text-center text-[13px] text-jb-ink/45 mb-4">
                Settling an outstanding tab
              </p>
              <PhoneFrame>
                <OutstandingBillPreview />
              </PhoneFrame>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-14 items-center">
          <div>
            <SectionIntro
              ...
              icon="offline"
              tag="OFFLINE MODE"
              title={"Your bar keeps moving,\neven when the network doesn't."}
            />
            <p className="text-sm text-jb-ink/55 leading-relaxed max-w-md">
              If the internet goes down, you can keep recording sales and other activity. Pending
              activity clearly shows it&apos;s waiting, and synchronizes as soon as the connection
              returns.
            </p>
            <p className="mt-4 text-sm text-jb-ink/40 leading-relaxed max-w-md">
              We keep this honest: your activity is kept safe on your device, but two disconnected
              phones won&apos;t always see the exact same numbers in the moment. justbarme flags
              anything that needs a second look once you&apos;re back online.
            </p>
            <div className="mt-6">
              <OfflineSyncBadge />
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <BentoCard className="p-6" delay={100}>
              ...
              <p className="text-[13px] text-jb-ink/60 leading-relaxed">
                Everything pending syncs automatically, and the app shows exactly what&apos;s still
                catching up.
              </p>
            </BentoCard>
          </div>
        ...
        <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-14 items-center">
          <div>
            ...
            <p className="mt-5 text-[13px] text-jb-ink/40 max-w-md">
              Different purchase prices over time are preserved, so old records never quietly
              change.
            </p>
          </div>
          <PhoneFrame>
      ...
      <section className="py-16 border-t border-jb-ink/[0.06] overflow-hidden select-none">
        ...
        <div
          className="flex border-y border-jb-ink/[0.06]"
          ...
          {Array.from({ length: 3 }).map((_, rep) => (
            <div key={rep} className="flex shrink-0">
              {AUDIENCE.map((a) => (
                <div
                  key={a}
                  className="flex items-center gap-3 px-8 py-5 border-r border-jb-ink/[0.06] shrink-0"
                >
                  <span className="w-1.5 h-1.5 rounded-full bg-jb-ink/20 shrink-0" />
                  <span className="text-sm text-jb-ink/50 whitespace-nowrap tracking-wide">
                    {a}
                  </span>
                </div>
              ))}
      ...
      <section className="py-24 md:py-32 px-6 md:px-12 lg:px-20 border-t border-jb-ink/[0.06]">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="relative">
            <div
              className="absolute left-[19px] top-2 bottom-2 w-px bg-jb-ink/10"
              aria-hidden="true"
            />
      ...
      <section className="py-24 md:py-32 px-6 md:px-12 lg:px-20 border-t border-jb-ink/[0.06] bg-jb-green-dark text-jb-cream">
        <div className="max-w-2xl mx-auto text-center">
          <h2
            className="text-3xl md:text-5xl font-light tracking-tight leading-[1.08] mb-6"
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
              to="/onboarding"
              className="px-8 py-3.5 rounded-xl bg-jb-cream text-jb-ink text-sm font-medium hover:bg-white transition-colors"
            >
              Create Your Business
            </Link>
            <a
              href="#features"
              className="px-8 py-3.5 rounded-xl border border-jb-cream/25 text-jb-cream text-sm font-medium hover:bg-jb-cream/10 transition-colors"
            >
              Explore Features
            </a>
          </div>
          <Link
            to="/install"
            className="mt-8 inline-block text-xs text-jb-cream/40 hover:text-jb-cream/70 tracking-wide transition-colors"
          >
      ...
      <footer className="py-10 px-6 md:px-12 lg:px-20 border-t border-jb-ink/[0.06]">
          ...
          <div className="flex flex-wrap items-center gap-x-8 gap-y-3">
            {[
              ...
              <a
                key={l.label}
                href={l.href}
                className="text-xs text-jb-ink/40 hover:text-jb-ink/80 transition-colors tracking-widest"
              >
                {l.label}
              </a>
            ))}
          ...
          <div className="flex items-center gap-6">
            {['Privacy', 'Terms', 'Help'].map((l) => (
              <a
                key={l}
                href="#"
                className="text-xs text-jb-ink/25 hover:text-jb-ink/55 transition-colors tracking-widest"
              >
                {l}
              </a>
            ))}
        ...
        <div className="max-w-6xl mx-auto mt-8 pt-6 border-t border-jb-ink/[0.04]">
          <span className="text-xs text-jb-ink/25">
            © 2026 justbarme. Built for bars and lounges.
          </span>
        </div>
      </footer>
    </div>
