...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    ...
    >
      <span className={`w-1.5 h-1.5 rounded-full ${active ? 'bg-emerald-500' : 'bg-jb-gold'}`} aria-hidden="true" />
      {status}
    </span>
  );
}
...
export function PlatformDashboard() {
  ...
  const [pendingAction, setPendingAction] = useState<{ businessId: string; suspend: boolean } | null>(null);
    ...
    <div className="min-h-screen bg-jb-cream text-jb-ink">
      <header className="border-b border-jb-ink/10 bg-white/60">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-pixel text-[10px] tracking-[0.2em] text-jb-ink/50">JUSTBARME PLATFORM</span>
          </div>
          <div className="flex items-center gap-3">
            <div className="text-right">
              <div className="text-[13px] text-jb-ink/80">{staff?.name}</div>
              <div className="text-[11px] text-jb-ink/40 uppercase tracking-wide">{staff?.role}</div>
            </div>
            <button
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-4">
          ...
          {!canSuspend && (
            <span className="text-[12px] text-jb-ink/40">Read-only — support role cannot suspend or reactivate.</span>
          )}
        </div>
        ...
        {businesses === null && !loadError && <p className="text-[13px] text-jb-ink/45">Loading…</p>}
                      ...
                      <tr key={`${b.id}-action`} className="bg-jb-ink/[0.03]">
                        <td colSpan={canSuspend ? 5 : 4} className="px-4 py-4">
                          <p className="text-[13px] text-jb-ink/70 mb-2">
                            {pendingAction.suspend ? 'Suspend' : 'Reactivate'} <strong>{b.name}</strong> — a reason is
        ...
        <div className="mt-8">
          ...
          {showAuditLog && (
            <div className="mt-3 rounded-xl border border-jb-ink/10 bg-white/60 divide-y divide-jb-ink/[0.06] overflow-hidden">
              {auditLog === null && <p className="px-4 py-3 text-[13px] text-jb-ink/45">Loading…</p>}
              ...
              {auditLog?.map((entry) => (
                <div key={entry.id} className="px-4 py-3 flex items-center justify-between gap-4">
                  <div>
                    ...
                    {entry.reason && <div className="text-[11px] text-jb-ink/40">{entry.reason}</div>}
                  </div>
                  <div className="text-[11px] text-jb-ink/40 shrink-0">{formatDateTime(entry.created_at)}</div>
                </div>
              ))}
            </div>
          )}
